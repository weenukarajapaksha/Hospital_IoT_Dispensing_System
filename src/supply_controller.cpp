...
void loop() {
  ...
  // In the Wokwi simulation, potentiometers represent remaining supply levels.
...
void handleRefillRequest(int waterLevel, int chemLevel, int tankLevel) {
  // The requested refill amount is based on the current dispensing tank level.
  ...
  lcd.setCursor(0, 1);
  lcd.print("Water + Chem    ");
  // Stage 1: dose purified water and concentrated chemical.
  digitalWrite(WATER_VALVE, HIGH);
  digitalWrite(CHEM_PUMP, HIGH);
  publishStatus("REFILL_STAGE:WATER_CHEM");
  delay(3000);
  mqtt.loop();
  ...
  lcd.setCursor(0, 1);
  lcd.print("Mixing...       ");
  // Stage 2: mix the water and chemical before transfer.
  digitalWrite(MIX_VALVE, HIGH);
  publishStatus("REFILL_STAGE:MIXING");
  delay(3000);
  mqtt.loop();

  lcd.setCursor(0, 1);
  lcd.print("To tank...      ");
  publishStatus("REFILL_STAGE:TRANSFER");

  // Stage 3: report transfer progress to the dispensing tank controller.
...
void publishStatus(const String &status) {
  mqtt.publish(TOPIC_REFILL_STATUS, status.c_str());
  Serial.print("MQTT sent: ");
  Serial.println(status);
}

void errorAlarm(const String &message) {
  // A fault always returns all actuators to their safe OFF state.
  stopAll();
